...
const steps = [
  ...
  { number: '03', icon: '🤖', title: 'L\'IA génère pour vous', desc: 'Groq AI crée vos flashcards, questions de quiz et résumés en quelques secondes.', color: 'from-emerald-500 to-green-600' },
  ...
  { number: '05', icon: '📊', title: 'Suivez vos progrès', desc: 'Le dashboard affiche votre courbe de mémorisation, streak et taux de maîtrise.', color: 'from-rose-500 to-pink-600' },
]
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="max-w-5xl mx-auto px-8 py-20">
        ...
        {/* Étapes */}
        <div className="mb-20">
          <h2 className="text-3xl font-black text-gray-900 dark:text-white text-center mb-4">Comment ça marche ?</h2>
          <p className="text-gray-500 dark:text-gray-400 text-center mb-12">5 étapes simples pour mémoriser efficacement</p>
          <div className="flex flex-col gap-6">
            {steps.map((step, i) => (
              <motion.div
                ...
                className="flex gap-6 items-start bg-white dark:bg-slate-900 rounded-3xl p-6 border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all"
              >
        ...
        {/* SM-2 */}
        <div className="bg-gradient-to-br from-violet-50 to-purple-50 dark:from-violet-500/10 dark:to-purple-500/10 rounded-3xl p-10 border border-violet-100 dark:border-violet-500/20 mb-20">
          <h2 className="text-2xl font-black text-violet-900 dark:text-violet-300 mb-4">🧠 L'algorithme SM-2 expliqué</h2>
          <p className="text-violet-700 dark:text-violet-400 mb-8 leading-relaxed">
            SM-2 est basé sur la courbe d'oubli d'Ebbinghaus. L'idée : réviser juste avant d'oublier.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { emoji: '😰', label: 'À revoir (0)', desc: 'La carte revient demain.', color: 'bg-red-100 dark:bg-red-500/20 border-red-200 dark:border-red-500/30' },
              { emoji: '🤔', label: 'Difficile (1)', desc: 'Revient dans 1-2 jours.', color: 'bg-amber-100 dark:bg-amber-500/20 border-amber-200 dark:border-amber-500/30' },
              { emoji: '😊', label: 'Facile (2)', desc: 'L\'intervalle augmente progressivement.', color: 'bg-emerald-100 dark:bg-emerald-500/20 border-emerald-200 dark:border-emerald-500/30' },
            ].map(grade => (
              <div key={grade.label} className={`${grade.color} border rounded-2xl p-5`}>
                <div className="text-3xl mb-3">{grade.emoji}</div>
                <p className="font-bold text-gray-800 dark:text-white mb-2">{grade.label}</p>
                <p className="text-gray-600 dark:text-gray-400 text-sm">{grade.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* FAQ */}
        <div className="mb-20">
          <h2 className="text-3xl font-black text-gray-900 dark:text-white text-center mb-12">Questions fréquentes</h2>
          <div className="flex flex-col gap-3">
            {faqs.map((faq, i) => (
              <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                >
                  <span className="font-bold text-gray-800 dark:text-white">{faq.q}</span>
                ...
                <AnimatePresence>
                  {openFaq === i && (
                    ...
                    >
                      <div className="px-6 pb-6 text-gray-600 dark:text-gray-400 leading-relaxed border-t border-gray-100 dark:border-slate-700 pt-4">
                        {faq.a}
                      </div>
                    </motion.div>
                  )}
        ...
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 rounded-3xl p-12 text-white text-center">
          ...
          <p className="text-white/70 mb-8 text-lg">Créez votre première matière et uploadez vos notes en moins de 2 minutes.</p>
